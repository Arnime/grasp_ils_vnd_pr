...
@testset "GIVPOptimizer.jl" begin
    ...
    @testset "TerminationReason" begin
        @test GIVPOptimizer.termination_from_message("converged successfully") == GIVPOptimizer.converged
        @test GIVPOptimizer.termination_from_message("time limit reached") == GIVPOptimizer.time_limit_reached
        @test GIVPOptimizer.termination_from_message("early stop triggered") == GIVPOptimizer.early_stop
        @test GIVPOptimizer.termination_from_message("threshold exceeded") == GIVPOptimizer.early_stop
        @test GIVPOptimizer.termination_from_message("no feasible solution") == GIVPOptimizer.no_feasible
        @test GIVPOptimizer.termination_from_message("no solution found") == GIVPOptimizer.no_feasible
        @test GIVPOptimizer.termination_from_message("max iterations reached") == GIVPOptimizer.max_iterations_reached
        @test GIVPOptimizer.termination_from_message("iteration limit") == GIVPOptimizer.max_iterations_reached
        @test GIVPOptimizer.termination_from_message("something else entirely") == GIVPOptimizer.unknown
    ...
    @testset "ElitePool" begin
        @testset "basic operations" begin
            ep = GIVPOptimizer.ElitePool(; max_size=3, min_distance=0.01,
        ...
        @testset "min_distance rejection" begin
            ep = GIVPOptimizer.ElitePool(; max_size=5, min_distance=0.5,
        ...
        @testset "replace worst when full" begin
            ep = GIVPOptimizer.ElitePool(; max_size=2, min_distance=0.01,
    ...
    @testset "GRASP" begin
        @testset "validate_bounds_and_initial!" begin
            ...
            @test_throws InvalidBoundsError GIVPOptimizer.validate_bounds_and_initial!([0.0], upper, nothing, 2)
            # upper length mismatch
            @test_throws InvalidBoundsError GIVPOptimizer.validate_bounds_and_initial!(lower, [10.0], nothing, 2)
            # initial_guess wrong length
            @test_throws InvalidInitialGuessError GIVPOptimizer.validate_bounds_and_initial!(lower, upper, [5.0], 2)
            # initial_guess out of bounds
            @test_throws InvalidInitialGuessError GIVPOptimizer.validate_bounds_and_initial!(lower, upper, [0.0, 5.0], 2)
            @test_throws InvalidInitialGuessError GIVPOptimizer.validate_bounds_and_initial!(lower, upper, [5.0, 10.0], 2)
        end
        ...
        @testset "evaluate_with_cache" begin
            GIVPOptimizer.set_integer_split!(2)
            # without cache
            @test GIVPOptimizer.evaluate_with_cache([1.0, 2.0], x -> sum(x.^2), nothing) == 5.0
            ...
            @test GIVPOptimizer.evaluate_with_cache([1.0, 2.0], x -> error("boom"), cache2) == Inf
        ...
        @testset "construct_grasp" begin
            ...
            sol = GIVPOptimizer.construct_grasp(4, lower, upper, sphere, nothing, 0.3;
                num_candidates_per_step=10)
            @test length(sol) == 4
            @test all(lower .<= sol .<= upper)

            # with initial guess
            sol2 = GIVPOptimizer.construct_grasp(4, lower, upper, sphere, [1.0, 1.0, 1.0, 1.0], 0.3;
                num_candidates_per_step=10)
            @test length(sol2) == 4

            # with cache
            cache = GIVPOptimizer.EvaluationCache()
            sol3 = GIVPOptimizer.construct_grasp(4, lower, upper, sphere, nothing, 0.3;
        ...
        @testset "construct_grasp with infeasible initial guess" begin
            ...
            lower = [0.0, 0.0]
            upper = [10.0, 10.0]
            sol = GIVPOptimizer.construct_grasp(2, lower, upper, bad_eval, [5.0, 5.0], 0.3;
    ...
    @testset "VND" begin
        ...
        @testset "try_continuous_move" begin
            ...
            sol2 = [3.0]
            GIVPOptimizer.try_continuous_move(1, sol2, cost_fn(sol2), cost_fn, rng2, nothing, nothing)
            GIVPOptimizer.set_integer_split!(nothing)
        end
        ...
        @testset "perturb_index! continuous" begin
            ...
            sol = [5.0, 5.0, 3.0, 3.0]
            # Continuous index (1)
            GIVPOptimizer.perturb_index!(sol, 1, 4, rng, [0.0, 0.0, 1.0, 1.0], [10.0, 10.0, 5.0, 5.0])
            @test sol[1] != 5.0
            @test 0.0 <= sol[1] <= 10.0
        ...
        @testset "perturb_index! integer" begin
            ...
            GIVPOptimizer.perturb_index!(sol, 3, 4, rng, [0.0, 0.0, 1.0, 1.0], [10.0, 10.0, 5.0, 5.0])
        ...
        @testset "neighborhood_flip" begin
            ...
            upper = [5.0, 5.0, 5.0, 5.0]
            new_sol, new_cost = GIVPOptimizer.neighborhood_flip(sphere, sol, sphere(sol), 4;
                lower=lower, upper=upper, seed=42)
            @test new_cost <= sphere(sol)

            # with sensitivity
            sens = [10.0, 0.1, 5.0, 0.5]
            new_sol2, new_cost2 = GIVPOptimizer.neighborhood_flip(sphere, sol, sphere(sol), 4;
                lower=lower, upper=upper, sensitivity=sens, seed=42)
            @test new_cost2 <= sphere(sol)

            # best improvement (first_improvement=false)
            new_sol3, new_cost3 = GIVPOptimizer.neighborhood_flip(sphere, sol, sphere(sol), 4;
        ...
        @testset "neighborhood_swap" begin
            ...
            lower = [-5.0, -5.0, 0.0, 0.0]
            upper = [5.0, 5.0, 5.0, 5.0]
            new_sol, new_cost = GIVPOptimizer.neighborhood_swap(sphere, sol, sphere(sol), 4;
            ...
            ns_sol, ns_cost = GIVPOptimizer.neighborhood_swap(flat_fn, [1.0, 1.0, 1.0, 1.0],
        ...
        @testset "neighborhood_multiflip" begin
            ...
            lower = [-5.0, -5.0, 0.0, 0.0]
            upper = [5.0, 5.0, 5.0, 5.0]
            new_sol, new_cost = GIVPOptimizer.neighborhood_multiflip(sphere, sol, sphere(sol), 4;
                lower=lower, upper=upper, seed=42)
            @test length(new_sol) == 4

            # without bounds
            new_sol2, _ = GIVPOptimizer.neighborhood_multiflip(sphere, sol, sphere(sol), 4;
                seed=42)
            @test length(new_sol2) == 4
            GIVPOptimizer.set_integer_split!(nothing)
        end
    ...
    @testset "ILS" begin
        @testset "perturb_solution" begin
            ...
            perturbed = GIVPOptimizer.perturb_solution(sol, 4; strength=4, seed=42,
                lower=lower, upper=upper)
        ...
        @testset "ils_search" begin
            ...
            best_sol, best_cost = GIVPOptimizer.ils_search(sol, sphere(sol), 4, sphere, config;
                lower=lower, upper=upper)
            ...
            cache = GIVPOptimizer.EvaluationCache()
            best_sol2, best_cost2 = GIVPOptimizer.ils_search(sol, sphere(sol), 4, sphere, config;
            ...
            best_sol5, best_cost5 = GIVPOptimizer.ils_search(sol, sphere(sol), 4, sphere, config;
                lower=lower, upper=upper, cache=tiny_cache)
            @test best_cost5 <= sphere(sol)

            # without cache (covers else branch: cost_fn(perturbed))
            best_sol4, best_cost4 = GIVPOptimizer.ils_search(sol, sphere(sol), 4, sphere, config;
                lower=lower, upper=upper)
            @test best_cost4 <= sphere(sol)

            # expired deadline
            best_sol3, best_cost3 = GIVPOptimizer.ils_search(sol, sphere(sol), 4, sphere, config;
                deadline=time() - 1.0)
    ...
    @testset "Path Relinking" begin
        @testset "path_relinking forward" begin
            ...
            source = [5.0, 5.0, 5.0]
            target = [0.1, 0.1, 0.1]
            best_sol, best_cost = GIVPOptimizer.path_relinking(sphere, source, target;
        ...
        @testset "path_relinking_best no improving move (break)" begin
            ...
            best_sol, best_cost = GIVPOptimizer.path_relinking(opt_fn, source, target;
                strategy=:best, seed=42)
        ...
        @testset "identical solutions" begin
            sphere(x) = sum(x.^2)
            source = [1.0, 2.0, 3.0]
            best_sol, best_cost = GIVPOptimizer.path_relinking(sphere, source, copy(source);
                seed=42)
            @test best_cost == sphere(source)
    ...
    @testset "Impl" begin
        @testset "evaluate_with_cache_impl" begin
            GIVPOptimizer.set_integer_split!(2)
            # without cache
            @test GIVPOptimizer.evaluate_with_cache_impl([1.0, 2.0], x -> sum(x.^2), nothing) == 5.0

            # with cache
            cache = GIVPOptimizer.EvaluationCache()
            @test GIVPOptimizer.evaluate_with_cache_impl([1.0, 2.0], x -> sum(x.^2), cache) == 5.0
            # cached hit
        ...
        @testset "do_path_relinking! skip conditions" begin
            ...
            bc, bs, st = GIVPOptimizer.do_path_relinking!(0, 10.0, [1.0], 0, cfg, nothing,
                x -> sum(x.^2), 1)
            ...
            bc2, _, _ = GIVPOptimizer.do_path_relinking!(0, 10.0, [3.0], 0, cfg2, ep,
                x -> sum(x.^2), 1)
            @test bc2 == 10.0  # skip because iteration=0

            # not on frequency
            bc3, _, _ = GIVPOptimizer.do_path_relinking!(3, 10.0, [3.0], 0, cfg2, ep,
                x -> sum(x.^2), 1)
        ...
        @testset "do_path_relinking! executes" begin
            ...
            bc, bs, st = GIVPOptimizer.do_path_relinking!(5, 100.0, [7.0, 7.0], 3, cfg, ep,
                sphere, 2; cache=GIVPOptimizer.EvaluationCache())
            @test bc <= 100.0
            GIVPOptimizer.set_integer_split!(nothing)
        end
        ...
        @testset "grasp_ils_vnd with all features" begin
            ...
            sol, cost = GIVPOptimizer.grasp_ils_vnd(sphere, 2, cfg;
        ...
        @testset "grasp_ils_vnd stagnation restart improves best" begin
            ...
            sol, cost = GIVPOptimizer.grasp_ils_vnd(flat_then_real, 2, cfg;
                lower=[0.0, 0.0], upper=[10.0, 10.0])
            @test isfinite(cost)
