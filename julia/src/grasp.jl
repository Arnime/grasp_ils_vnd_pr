...
function evaluate_with_cache(cand::Vector{Float64}, evaluator::Function,
...
function select_from_rcl(costs::Vector{Float64}, alpha::Float64, rng::AbstractRNG)::Union{Int,Nothing}
...
function sample_integer_from_bounds(lower::Float64, upper::Float64, rng::AbstractRNG)::Float64
    lo = ceil(Int, lower)
    hi = floor(Int, upper)
...
function build_random_candidate(num_vars::Int, half::Int,
...
function build_heuristic_candidate(num_vars::Int, half::Int,
                                   lower::Vector{Float64}, upper::Vector{Float64},
                                   rng::AbstractRNG)::Vector{Float64}
    sol = Vector{Float64}(undef, num_vars)
    ...
    for idx in (half+1):num_vars
        ...
        if hi > lo && cont_idx <= half && span[cont_idx] > 0
            ...
            sol[idx] = hi >= lo ? Float64(hi) : Float64(round(Int, (lower[idx] + upper[idx]) / 2.0))
        end
    end
    return sol
end

function construct_grasp(num_vars::Int, lower::Vector{Float64}, upper::Vector{Float64},
                         evaluator::Function, initial_guess::Union{Vector{Float64},Nothing},
                         alpha::Float64;
                         seed::Union{Int,Nothing}=nothing,
                         num_candidates_per_step::Int=20,
                         cache::Union{EvaluationCache,Nothing}=nothing,
                         n_workers::Int=1)::Vector{Float64}
    rng = new_rng(seed)
    ...
    if initial_guess !== nothing
        ...
        push!(costs, evaluate_with_cache(seed_candidate, evaluator, cache))
    end
    ...
    push!(costs, evaluate_with_cache(heur, evaluator, cache))
    ...
    while length(candidates) < n_candidates
        ...
        push!(candidates, sol)
        push!(costs, evaluate_with_cache(sol, evaluator, cache))
    end
