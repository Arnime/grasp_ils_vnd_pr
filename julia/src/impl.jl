...
function evaluate_with_cache_impl(sol::Vector{Float64}, cost_fn::Function,
                                  cache::Union{EvaluationCache,Nothing})::Float64
    if cache !== nothing
...
function do_path_relinking!(iteration::Int, best_cost::Float64, best_solution::Vector{Float64},
                            stagnation::Int, config::GIVPConfig,
                            elite_pool::Union{ElitePool,Nothing},
                            cost_fn::Function, num_vars::Int;
                            cache::Union{EvaluationCache,Nothing}=nothing,
                            deadline::Float64=0.0)
    if !(config.use_elite_pool && elite_pool !== nothing &&
         iteration > 0 && iteration % config.path_relink_frequency == 0 &&
    ...
    for i in 1:min(3, length(elite_solutions))
        for j in (i+1):min(4, length(elite_solutions))
            ...
            pr_solution, _ = bidirectional_path_relinking(cached_fn, source, target; deadline)
            pr_solution = local_search_vnd(cached_fn, pr_solution, num_vars;
                max_iter=config.vnd_iterations ÷ 2, cache, deadline)
...
function grasp_ils_vnd(cost_fn::Function, num_vars::Int, config::GIVPConfig;
                       verbose::Bool=false,
    ...
    elite_pool = config.use_elite_pool ?
        ElitePool(; max_size=config.elite_size, lower, upper) : nothing
    cache = config.use_cache ? EvaluationCache(; maxsize=config.cache_size) : nothing
    conv_monitor = config.use_convergence_monitor ?
    ...
    verbose && @info @sprintf("GRASP-ILS-VND-PR start | n=%d | iters=%d | alpha=[%.3f, %.3f] | elite=%d",
    ...
    for iteration in 0:(config.max_iterations - 1)
        if expired(deadline)
            verbose && @info @sprintf("TIME LIMIT: %.0fs reached at iteration %d",
        ...
        # GRASP construction
        sol = construct_grasp(num_vars, lower, upper, cost_fn, initial_guess, current_alpha;
            num_candidates_per_step=config.num_candidates_per_step,
            cache, n_workers=config.n_workers)

        # VND local search
        sol = local_search_vnd(cost_fn, sol, num_vars;
            max_iter=config.vnd_iterations, lower, upper, cache, deadline)

        # ILS
        vnd_cost = evaluate_with_cache_impl(sol, cost_fn, cache)
        sol, vnd_cost = ils_search(sol, vnd_cost, num_vars, cost_fn, config;
        ...
        best_cost, best_solution, stagnation = do_path_relinking!(
            iteration, best_cost, best_solution, stagnation, config,
            elite_pool, cost_fn, num_vars; cache, deadline)

        # Verbose logging
        if verbose
            elapsed = time() - start_time
            marker = cost < original_best ? "*" : " "
            @info @sprintf("%s iter %3d/%d | cur=%12.4f | best=%12.4f | alpha=%.3f | stag=%3d | elite=%2d | t=%6.2fs",
                marker, iteration + 1, config.max_iterations, cost, best_cost,
                current_alpha, stagnation,
                elite_pool !== nothing ? pool_size(elite_pool) : 0, elapsed)
        end

        # Stagnation restart
        if stagnation > config.max_iterations ÷ 4
            verbose && @info @sprintf("Stagnation detected (%d iter without improvement) — partial restart", stagnation)
            restart_rng = new_rng()
            ...
            restart_arr = local_search_vnd(cost_fn, restart_arr, num_vars;
                max_iter=config.vnd_iterations, lower, upper, cache, deadline)
            restart_cost = cost_fn(restart_arr)
            restart_arr, restart_cost = ils_search(restart_arr, restart_cost, num_vars, cost_fn, config;
                lower, upper, cache, deadline)
        ...
        # Early stopping
        if conv_monitor !== nothing && conv_monitor.no_improve_count >= config.early_stop_threshold
            verbose && @info @sprintf("EARLY STOP: %d iterations without improvement",
    ...
    if verbose
        elapsed = time() - start_time
        @info @sprintf("GRASP-ILS-VND-PR end | best=%.4f | stagnation=%d | t=%.2fs",
            best_cost, stagnation, elapsed)
        if cache !== nothing
            stats = cache_stats(cache)
            @info @sprintf("Cache Stats: %d hits, %d misses, rate=%.1f%%, size=%d",
