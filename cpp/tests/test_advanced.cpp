...
TEST_CASE("all-integer problem (integer_split = 0)", "[mixed]") {
  ...
  for (std::size_t i = 0; i < result.x.size(); ++i)
    REQUIRE(result.x[i] == Catch::Approx(std::round(result.x[i])).margin(1e-9));
}
...
TEST_CASE("path relinking 30D problem exercises diff-variable cap",
          "[advanced]") {
...
TEST_CASE("path relinking with identical elite solutions is a no-op",
          "[advanced]") {
  ...
  // current cost immediately without iterating — exercises the empty-diff
  // branch.
  //
...
TEST_CASE("non-adaptive alpha uses fixed alpha throughout", "[advanced]") {
  ...
  // instead of interpolating — exercises the `if (!adaptive) return alpha;`
  // branch.
  std::vector<std::pair<double, double>> bounds(5, {-5.0, 5.0});
  GivpConfig cfg;
  cfg.seed = 42;
...
TEST_CASE("integer bounds narrower than 1 integer triggers fallback",
          "[advanced]") {
...
TEST_CASE("path relinking skipped when elite pool has < 2 solutions",
          "[advanced]") {
...
TEST_CASE(
    "vnd multiflip neighborhood triggered after 5 consecutive flip failures",
    "[advanced]") {
  // Flat objective → neighborhood_flip never improves → no_improve_flip_count
  // reaches 5 within the first 6 VND iterations, triggering
  // neighborhood_multiflip.
  auto flat = [](const std::vector<double> &) -> double { return 1.0; };
  std::vector<std::pair<double, double>> bounds(4, {-5.0, 5.0});
  GivpConfig cfg;
  cfg.seed = 42;
