...
TEST_CASE("default config is valid", "[config]") { REQUIRE_NOTHROW(GivpConfig{}.validate()); }
...
TEST_CASE("empty bounds throws InvalidBounds", "[config]") {
    REQUIRE_THROWS_AS(givp::givp([](const std::vector<double> &) { return 0.0; }, {}, GivpConfig{}),
                      InvalidBounds);
}
...
TEST_CASE("inverted bounds throws InvalidBounds", "[config]") {
    ...
    REQUIRE_THROWS_AS(
        givp::givp([](const std::vector<double> &) { return 0.0; }, bad, GivpConfig{}),
        InvalidBounds);
}
...
TEST_CASE("termination_from_message parses known strings", "[config]") {
    ...
    REQUIRE(termination_from_message("time limit reached") == TerminationReason::TimeLimitReached);
    REQUIRE(termination_from_message("early stop due to stagnation") ==
            TerminationReason::EarlyStop);
    REQUIRE(termination_from_message("converged") == TerminationReason::Converged);
    REQUIRE(termination_from_message("no feasible solution found") ==
            TerminationReason::NoFeasible);
    REQUIRE(termination_from_message("") == TerminationReason::Unknown);
}
