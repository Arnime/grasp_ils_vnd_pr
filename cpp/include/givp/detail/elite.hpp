...
public:
    ElitePool(std::size_t max_size, double min_distance,
    ...
    std::pair<const std::vector<double>*, double> get_best() const {
        if (pool_.empty()) throw EmptyPool("elite pool is empty");
        return {&pool_.front().first, pool_.front().second};
    }

    const std::vector<std::pair<std::vector<double>, double>>& get_all() const {
        return pool_;
    }
    ...
    void keep_top(std::size_t n) {
        if (pool_.size() > n) pool_.resize(n);
    }
};
