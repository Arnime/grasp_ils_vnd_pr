...
static std::vector<double> perturb_solution(
    const std::vector<double>& solution, std::size_t half,
    std::size_t strength,
    const std::vector<double>& lower, const std::vector<double>& upper,
    RngT& rng) {

    std::size_t n          = solution.size();
    std::size_t num_perturb = std::max(std::size_t{1},
                                        std::min(strength, n / 5));
    ...
    for (std::size_t k = 0; k < num_perturb; ++k) {
        ...
        if (idx >= half) {
            ...
            perturbed[idx] = std::round(
                clamp_val(perturbed[idx] + delta, lower[idx], upper[idx]));
        } else {
            double span  = upper[idx] - lower[idx];
            double delta = rng.uniform(-0.15, 0.15) * span;
            perturbed[idx] = clamp_val(perturbed[idx] + delta,
                                        lower[idx], upper[idx]);
        }
    }
...
double ils_search(
    ...
    std::size_t half,
    const std::vector<double>& lower, const std::vector<double>& upper,
    std::size_t ils_iterations, std::size_t vnd_iterations,
    std::size_t perturbation_strength,
    ...
    for (std::size_t i = 0; i < ils_iterations; ++i) {
        ...
        double progress = static_cast<double>(i) /
                          static_cast<double>(std::max(ils_iterations, std::size_t{1}));
        std::size_t effective_strength = std::max(
            perturbation_strength,
            static_cast<std::size_t>(
                static_cast<double>(perturbation_strength) * (1.0 + progress)));

        auto candidate =
            perturb_solution(best_sol, half, effective_strength, lower, upper, rng);
        double perturbed_cost =
            evaluate_with_cache(candidate, func, cache, half);
        double vnd_cost =
            local_search_vnd(func, candidate, perturbed_cost, half,
                              lower, upper, vnd_iterations, cache, rng, deadline);
