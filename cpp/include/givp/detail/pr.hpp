...
path_relinking_best(const F &func, const std::vector<double> &source,
    ...
    while (!diff_indices.empty()) {
        ...
        diff_indices.erase(diff_indices.begin() + static_cast<std::ptrdiff_t>(best_idx_pos));
...
std::pair<std::vector<double>, double> bidirectional_path_relinking(
    const F &func, const std::vector<double> &sol1, const std::vector<double> &sol2,
    std::size_t half, std::optional<EvaluationCache> &cache, RngT &rng, const Deadline &deadline) {
    ...
    if (diff_indices.size() > MAX_PR_VARS) {
        std::sort(diff_indices.begin(), diff_indices.end(), [&](std::size_t a, std::size_t b) {
            return std::abs(sol1[b] - sol2[b]) < std::abs(sol1[a] - sol2[a]);
        });
