...
OptimizeResult run(F&& func,
    ...
    if (config.use_cache) cache.emplace(config.cache_size);

    ElitePool elite_pool{config.elite_size, 0.05, lower, upper};
    std::optional<ConvergenceMonitor> conv_monitor;
    if (config.use_convergence_monitor) conv_monitor.emplace(20, 50);
    ...
    for (std::size_t iteration = 0; iteration < config.max_iterations; ++iteration) {
        if (expired(deadline)) { message = "time limit reached"; break; }
        ...
        // Convergence monitor — restart check
        if (conv_monitor) {
        ...
        // Convergence monitor — early stop check
        if (conv_monitor) {
            auto sig = conv_monitor->update(best_cost, &elite_pool);
            if (sig.no_improve_count >= config.early_stop_threshold) {
