...
make_elite_pool <- function(max_size = 7L, direction = "minimize") {
  structure(
    list(max_size = as.integer(max_size), direction = direction, items = list()),
...
elite_add <- function(pool, x, value) {
  ...
  ord <- order(vapply(pool$items, function(it) it$value, numeric(1L)),
...
elite_best <- function(pool) {
  if (length(pool$items) == 0L) return(NULL)
  pool$items[[1L]]
}