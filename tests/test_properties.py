...
# Keep the per-property budget small: each example runs the full optimizer.
_FAST = settings(
    max_examples=15,
    deadline=None,
...
def test_sphere_returns_solution_within_bounds(bounds: list[tuple[float, float]]):
    ...
    cfg = GraspIlsVndConfig(max_iterations=2, vnd_iterations=3)
    result = grasp_ils_vnd_pr(sphere, bounds, config=cfg)
...
def test_direction_flip_preserves_magnitude(bounds: list[tuple[float, float]]):
    ...
    cfg = GraspIlsVndConfig(max_iterations=2, vnd_iterations=3)
    res_min = grasp_ils_vnd_pr(f, bounds, direction="minimize", config=cfg)
    res_max = grasp_ils_vnd_pr(f, bounds, direction="maximize", config=cfg)
...
def test_initial_guess_outside_bounds_raises(
    ...
    with pytest.raises(InvalidInitialGuessError):
        grasp_ils_vnd_pr(f, bounds, initial_guess=bad_guess)
...
def test_nfev_matches_or_exceeds_iterations(bounds: list[tuple[float, float]]):
    ...
    cfg = GraspIlsVndConfig(max_iterations=3, vnd_iterations=2)
    result = grasp_ils_vnd_pr(f, bounds, config=cfg)
